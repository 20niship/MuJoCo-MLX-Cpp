#pragma once
// Vendored from 20niship/mlx-cross-platform tests/mujoco/euler_devmem.hpp at commit 3202339e12231f3a68f9ee67e0b6085a33c52286.
...
inline std::string euler_devmem_fmt_float(float x) {
...
inline mkx::fast::Kernel<> make_euler_devmem_kernel(int nv, int nq, float dt, const std::vector<int>& simple_qa, const std::vector<int>& simple_da, const std::vector<std::pair<int, int>>& free_joints, const std::vector<std::pair<int, int>>& ball_joints, const std::vector<float>& dof_damping_vals) {
  int n              = nv;
  std::string dt_str = euler_devmem_fmt_float(dt);
  std::ostringstream ss;
  ...
  for(int i = 0; i < nv; i++) {
    if(dof_damping_vals[static_cast<size_t>(i)] != 0.0f) {
      ss << "L_scratch[moff + " << i * n + i << "] += " << dt_str << "f * " << euler_devmem_fmt_float(dof_damping_vals[static_cast<size_t>(i)]) << "f;\n";
    }
  }
